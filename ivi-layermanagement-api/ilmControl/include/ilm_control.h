...
/**
 * \brief Take a screenshot from the current displayed layer scene.
 * The screenshot return immage data buffer.
 * \ingroup ilmControl
 * \param[in] screen Id of screen where screenshot should be taken
 * \param[in] doneCb pointer of callback when shot done and return buffer
 * \return ILM_SUCCESS if the method call was successful
 * \return ILM_FAILED if the client can not call the method on the service.
 */
ilmErrorTypes ilm_takeScreenshot_buffer(t_ilm_uint screen, shotdoneNotificationFunc doneCb);

/**
 * \brief Take a screenshot of a certain surface
 * The screenshot The screenshot return immage data buffer..
 * \ingroup ilmControl
 * \param[in] surfaceid Identifier of the surface to take the screenshot of
 * \param[in] doneCb pointer of callback when shot done and return buffer
 * \return ILM_SUCCESS if the method call was successful
 * \return ILM_FAILED if the client can not call the method on the service.
 */
ilmErrorTypes ilm_takeSurfaceScreenshot_buffer(t_ilm_surface surfaceid, shotdoneNotificationFunc doneCb);
