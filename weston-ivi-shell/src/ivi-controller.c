...
screen_listener_t screen_info_property_change_callback_listener = NULL;
...
check_surface_id(struct wl_client *client, uint32_t surface_id)
{
    ...
    if ((surface_id >> windowIdBitNum) == pid) {
        return 0;
    }
...
send_surface_prop(struct wl_listener *listener, void *data)
{
    ...
    if (screen_info_property_change_callback_listener) {
        screen_info_property_change_callback_listener();
    }
}
...
controller_set_surface_opacity(struct wl_client *client,
                   ...
                   wl_fixed_t opacity)
{
    if (check_surface_id(client, surface_id)) {
        ivi_wm_send_surface_error(resource, surface_id, 0, "pid check falied!!!");
        return;
    }
                              ...
                              uint32_t surface_id)
{
    ...
    ivi_screenshot_send_error(screenshot, 0, "screenshot disabled");
    return;
...
static int
load_screen_info_module(struct ivishell *shell)
{
    struct weston_config *config = wet_get_config(shell->compositor);
    struct weston_config_section *section;
    char *screen_info_module = NULL;

    int (*screen_info_module_init)(struct ivishell *shell);

    section = weston_config_get_section(config, "ivi-shell", NULL, NULL);

    if (weston_config_section_get_string(section, "screen-info-module",
                                         &screen_info_module, NULL) < 0) {
        /* input events are handled by weston's default grabs */
        weston_log("ivi-controller: No screen-info-module set\n");
        return 0;
    }

    screen_info_module_init = wet_load_module_entrypoint(screen_info_module, "ivishell_module_init");
    if (!screen_info_module_init)
        return -1;

    if (screen_info_module_init(shell) != 0) {
        weston_log("ivi-controller: Initialization of screen-info-module fails");
        return -1;
    }

    free(screen_info_module);
...
void
set_screen_listener(screen_listener_t listener)
{
    screen_info_property_change_callback_listener = listener;
}
		       ...
		       int *argc, char *argv[])
{
    ...
    if (load_screen_info_module(shell) < 0) {
        destroy_screen_ids(shell);
        free(shell);
        return -1;
    }
