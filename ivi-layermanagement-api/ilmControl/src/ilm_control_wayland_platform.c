...
struct screenshot_context {
    ...
    shotdoneNotificationFunc done_cb;
};

struct screenshot_buffer_context {
    bool isshotdone;
    ilmErrorTypes result;
    shotdoneNotificationFunc done_cb;
};
...
ilmControl_destroy(void)
{
    ...
    // OHOS fix: ctx->thread maybe less than 0 and it's right
...
static void screenshot_buffer_done(void *data, struct ivi_screenshot *ivi_screenshot,
                            int32_t fd, int32_t width, int32_t height,
                            int32_t stride, uint32_t format, uint32_t timestamp)
{
    struct screenshot_buffer_context *ctx_scrshot = data;
    char *buffer;
    size_t size = stride * height;

    ctx_scrshot->isshotdone = true;
    ivi_screenshot_destroy(ivi_screenshot);
    ...
    if (ctx_scrshot->done_cb) {
        ctx_scrshot->done_cb(buffer, size, width, height, format);
        ctx_scrshot->result = ILM_SUCCESS;
    } else {
        ctx_scrshot->result = ILM_FAILED;
    }
...
static void screenshot_buffer_error(void *data, struct ivi_screenshot *ivi_screenshot,
                             uint32_t error, const char *message)
{
    struct screenshot_buffer_context *ctx_scrshot = data;
    ctx_scrshot->isshotdone = true;
...
static struct ivi_screenshot_listener screenshot_buffer_listener = {
    screenshot_buffer_done,
    screenshot_buffer_error,
};

ILM_EXPORT ilmErrorTypes
ilm_takeScreenshot_buffer(t_ilm_uint screen, shotdoneNotificationFunc doneCb)
{
    ilmErrorTypes returnValue = ILM_FAILED;
    struct ilm_control_context *const ctx = &ilm_context;
    struct screen_context *ctx_scrn = NULL;
    ...
    if (ctx_scrn != NULL) {
        struct screenshot_buffer_context ctx_scrshot = {
            .isshotdone = false,
            .result = ILM_FAILED,
            .done_cb = doneCb,
        };
        ...
        if (scrshot) {
            ivi_screenshot_add_listener(scrshot, &screenshot_buffer_listener,
                                        &ctx_scrshot);
            ...
            } while ((ret != -1) && !ctx_scrshot.isshotdone);
...
ILM_EXPORT ilmErrorTypes
ilm_takeSurfaceScreenshot_buffer(t_ilm_surface surfaceid,
                              shotdoneNotificationFunc doneCb)
{
    ilmErrorTypes returnValue = ILM_FAILED;
    struct ilm_control_context *const ctx = &ilm_context;
